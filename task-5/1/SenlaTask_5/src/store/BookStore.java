...
public class BookStore {
    ...
    private static List<Book> requestList = new ArrayList<Book>(); //Список запрошенных книг и колчисетво запросов к каждой

    public static void leaveRequest(Book book) {
        if (!requestList.contains(book))
            requestList.add(book);
        book.setAmountRequest(book.getAmountRequest() + 1);
    }
    ...
    public List<Book> sortRequestByAmount(List<Book> unsorted) {
        List<Book> sorted = new ArrayList<>(unsorted);
        sorted.sort(new Comparator<Book>() {
            @Override
            public int compare(Book book1, Book book2) {
                return book1.getAmountRequest().compareTo(book2.getAmountRequest());
            }
        });
        return sorted;
    }

    public List<Book> sortRequestByTitle(List<Book> unsorted) {
    ...
    public List<Book> sortRequestByAuthor(List<Book> unsorted) {
        List<Book> sorted = new ArrayList<Book>();
        sorted.sort(new Comparator<Book>() {
            @Override
            public int compare(Book book1, Book book2) {
    ...
    public static List<Book> getRequestList() {
        return requestList;
    }
